function solve(data) {
    ...
    function sortSystemComparator(sysA, sysB, catalogue) {
        ...
        let firstComponents = catalogue.get(sysA).size;

        let secondComponents = catalogue.get(sysB).size;

        if (firstComponents > secondComponents) return -1;
    ...
    for (let system of systems) {
        ...
        let components = [...catalogue.get(system).keys()].sort((s1, s2) =>
            ...
            catalogue.get(system).get(s2).length - catalogue.get(system).get(s1).length);
...
solve(["SULS | Main Site | Home Page", "SULS | Main Site | Login Page", "SULS | Main Site 
    | Register Page", "SULS | Judge Site | Login Page", "SULS | Judge Site | Submittion Page"
    , "Lambda | CoreA | A23", "SULS | Digital Site | Login Page", "Lambda | CoreB | B24", "Lambda 
    | CoreA | A24", "Lambda | CoreA | A25", "Lambda | CoreC | C4", "Indice | Session | Default Storage", 
